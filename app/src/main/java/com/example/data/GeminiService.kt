...
object GeminiClient {
    ...
    suspend fun generateCommunityTasks(userTitle: String): List<String> {
        val apiKey = BuildConfig.GEMINI_API_KEY
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            // Return high-quality local fallbacks if API key is not configured yet
        ...
        val prompt = """
            ...
            Ne mets aucune balise markdown, pas de ```json, pas d'explication, seulement le tableau JSON.
        """.trimIndent()
    ...
    private fun parseJsonArray(rawJson: String): List<String> {
        ...
        return try {
            ...
            adapter.fromJson(cleaned) ?: getFallbackTasks("Artisan")
        } catch (e: Exception) {
            e.printStackTrace()
            // Split manually as a fallback
            cleaned.removePrefix("[").removeSuffix("]")
                .split(",")
                .map { it.trim().removeSurrounding("\"").removeSurrounding("'") }
                .filter { it.isNotEmpty() }
                .ifEmpty { getFallbackTasks("Artisan") }
        }
    }

    private fun getFallbackTasks(userTitle: String): List<String> {
    ...
    suspend fun generateMemberQuest(memberName: String, memberTitle: String): Pair<String, Int> {
        val apiKey = BuildConfig.GEMINI_API_KEY
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
        ...
        val prompt = """
            Génère une quête féodale médiévale amusante, immersive et unique pour un membre de la communauté nommé '$memberName' qui possède le rang/titre de '$memberTitle'.
            La quête doit être directement liée à son rang. Par exemple, un Chevalier s'entraîne ou combat, un Esclave fait des corvées ingrates, un Roi prend des décisions royales, etc.
            ...
            Ne mets aucune balise markdown, pas de ```json, pas d'explication, seulement l'objet JSON.
        """.trimIndent()
